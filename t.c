...
enum Objet table[Height][Width] =
    {
        ...
        {M, M, M, M, P, M, M, M, P, M, P, M, M, M, P, M, M, P, M},
        ...
        {M, M, M, M, P, M, V, M, M, W, M, M, V, M, P, M, M, M, M},
        ...
        {M, P, P, M, P, P, P, P, P, P, P, P, P, P, P, M, P, W, M},
...
//prints the score char to the screen after converting the score value to the correct characters

int put_score_into_fbuf(char * score,int *scoreValue,int j, int k)
{
   int i;
   for (i = 0; i < 12; i++)
   {
      unkpchar(score[i], j, k + i);
   }
   if (*scoreValue < 100)
   {
   ...
   if(*scoreValue >= 1000){
      score[8] = '0' + (((*scoreValue / 10) / 10) / 10);
      score[9] = '0' + (((*scoreValue / 10) / 10) %10);
      score[10] = '0' + ((*scoreValue / 10) %10);
      score[11] = '0' + (*scoreValue % 10);
   }

   for (i = 0; i < 12; i++)
   {
      kpchar(score[i], j, k + i);
   }
}
   ...
   ************/
   ...
   for (int j = 0; j < Height; j++)
      for (int i = 0; i < Width; i++)
      {
         ...
         if(table[j][i] == W){
            show_bmp1(&_binary_power_bmp_start, j * 16, 16 * i);
         }
      }
   show_bmp1(p, 0, 80);
   ...
   show_bmp1(&_binary_power_bmp_start, 16 * 8, 16 * 9);
   table[8][9] = W;
   ...
   put_score_into_fbuf(&score,&scoreValue,1,68);  
   while (1)
   {
      ...
      if (move && !dead)
      {
         ...
         //innovation - multiple the score value by 3 when encountering point that is set to W
         else if (table[y >> 4][x >> 4] == W)
         {

            uprintf("\nPOWERUP COLLECTED");
            uprintf("\nSCORE MULTIPLIED BY 3!");
         ...
         show_bmp(p, y, x, buff, replacePix, &oldstartR, &oldstartC);
         //checks if player moves into ghost 
         for (int l = 0; l < 3; l++)
         {
            if (sprites[l].x == x && sprites[l].y == y)
            {
              ...
              uprintf("YOURE DEAD!!");
               ...
               timer_stop(0);
               game_over_screen();
               put_score_into_fbuf(&score,&scoreValue,13,34);
      ...
      if (spriteMove && !dead)
      {
         for (int l = 0; l < 3; l++)
         {
            //checks if the ghosts x and y is equal to the players position
            if (sprites[l].x == x && sprites[l].y == y)
            {
               if (!dead)
                  dead = 1;
               //prints death message to console
               uprintf("YOURE DEAD!!");
               //stops timer
               timer_stop(0);
               game_over_screen();
            ...
            if (!sprites[l].path)
            {
               ...
               //
               //vertex of pacman and ghosts
               int pacman_vertex = (y >> 4) * Width + (x >> 4);
               int enemy_vertex = (((sprites[l].y) >> 4) * Width) + ((sprites[l].x) >> 4);

               dijkstra(graph, enemy_vertex, pacman_vertex);

...
int oldstartC;
int replacePix =0;
int putback(int buf[16][16],int oldstartRow, int oldstartCol){
    int h=16,w=16;
    int i,j;
    int x,y;
     int yy=0,xx=0;
    ...
    for (yy=0,i=oldstartRow; i<h+oldstartRow; i++,yy++){
     // for (j=startCol; j<w+startCol; j++){
     for (xx=0,j=oldstartCol; j<oldstartCol+w; j++,xx++){
// TODO
// save old fb value before modification
        fb[i*640 + j] = buf[yy][xx];
     }
   }

}

int show_bmp(char *p, int startRow, int startCol,int buf[16][16],int rePix,int *oldstartRow,int *oldstartCol)
{ 
 ...
 int yy=0,xx=0;
   if (rePix)
      putback(buf,*oldstartRow,*oldstartCol);
   ...
   // p += h*r2;
   p += (h-1)*r2;
   for (yy=0,i=startRow; i<h+startRow; i++,yy++){
     pp = p;
     // for (j=startCol; j<w+startCol; j++){
     for (xx=0,j=startCol; j<startCol+w; j++,xx++){
         b = *pp; g = *(pp+1); r = *(pp+2);
         pixel = (b<<16) + (g<<8) + r;
 /* TODO */
        buf[yy][xx] = fb[i*640 + j];
         fb[i*640 + j] = pixel;
...
int black_point(int oldstartRow, int oldstartCol){ 
    int h=16,w=16;
    int i,j;
    int x,y;

      for(y=oldstartCol, i=oldstartRow; i<h+oldstartRow; i++, y++){
        for(x=oldstartRow, j=oldstartCol; j<w+oldstartCol; j++, x++){
        clrpix(x,y);
        }
      }
}
...
int game_over_screen(){
  ...
  dstring("GAME OVER",17*16,12*16);
}

int highest_score(char * score,int *scoreValue){
  

  int i;
   for (i = 0; i < 12; i++)
   {
      unkpchar(score[i], 1, 68 + i);
   }
   if (*scoreValue < 100)
   {
      score[12] = '0' + (*scoreValue / 10);
      score[13] = '0' + (*scoreValue % 10);
   }
   if (*scoreValue >= 100)
   {
      score[12] = '0' + ((*scoreValue / 10) / 10);
      score[13] = '0' + ((*scoreValue / 10) % 10);
      score[14] = '0' + (*scoreValue % 10);
   }
   if(*scoreValue >= 1000){
      score[12] = '0' + (((*scoreValue / 10) / 10) / 10);
      score[13] = '0' + (((*scoreValue / 10) / 10) %10);
      score[14] = '0' + ((*scoreValue / 10) %10);
      score[15] = '0' + (*scoreValue % 10);
   }
  
   for (i = 0; i < 15; i++)
   {
      kpchar(score[i], 30, 12+i);
   }
}
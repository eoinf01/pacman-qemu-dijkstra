   ...
   while (1)
   {
      ...
      if (spriteMove)
      {
         ...
         if (!sprites[0].path)
         {
            ...
            uprintf("Vertices of pac: %d\nVertices of ghost: %d", vpac, vghost);
            dijkstra(graph, vghost, vpac);
            
            if (solnlen)
            {
               uprintf("Solution length: %d", solnlen);
               // copy the solution to the ghost's path list
               for (i = 0; i < solnlen; i++){
                  sprites[0].pathV[i] = soln[i];
                  uprintf("PREDICTED PATH: X%d",soln[i]);
               }
        ...
        */
            ...
            // uprintf("Vertex: %d", vertex);
               for(int k=0;k<Height;k++){
                  for(int j=0;j<Width;j++){
                     if( ((k) * Width + (j)) == vertex){
                        uprintf("Next Vertex: %d",((k >> 4) * Width + (j >> 4)));
                        // uprintf("X: %d Y: %d",sprites[0].x,sprites[0].y );
                        sprites[0].x = j*16;
                        sprites[0].y = k*16;
                     }
                  }
               }
               // sprites[0].x = (sprites[0].pathV[pos] % Width);
               // sprites[0].y = (sprites[0].pathV[pos] / Width);
               // uprintf("X: %d Y: %d",sprites[0].x,sprites[0] );
               sprites[0].pathpos = 1;
         ...
         else if (sprites[0].pathpos == sprites[0].pathlen)
         ...
         else if (sprites[0].pathpos < sprites[0].pathlen)
         {
           ...
           */
            ...
            //       for(int j=0;j<Width;j++){
            //          if( ((i) * Width + (j)) == vertex){
            //             uprintf("Next Vertex: %d",((i >> 4) * Width + (j >> 4)));
            //             sprites[0].x = i<<4;
            //             sprites[0].y = j<<4;
            //          }
            //       }
            ...
            for(int k=0;k<Height;k++){
                  for(int j=0;j<Width;j++){
                     if( ((k) * Width + (j)) == vertex){
                        uprintf("Next Vertex: %d",((j >> 4) * Width + (k >> 4)));
                        // uprintf("X: %d Y: %d",sprites[0].x,sprites[0].y );
                        sprites[0].x = j*16;
                        sprites[0].y = k*16;
                     }
